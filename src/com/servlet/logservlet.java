...
public class logservlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		manager.setManager_password(manager_pwd);
		sqlHelper sql = new sqlHelper();		
		Manager manager2 = sql.queManagerbyIDandPWD(manager);
		if(manager2.getManager_name()==null){
			response.sendRedirect("login.jsp?msg=2");
		}
		else{
			HttpSession session = request.getSession();
			session.setAttribute("manager_id", manager.getManager_id());
			session.setAttribute("name", manager2.getManager_name());
			session.setAttribute("islogin", "login_success");
